...
// button
 ...
 if(limiterR<=moduleSize)
  ...
  if(mods[incPaddle-1].rectHight>mods[incPaddle].rectHight)
  {
   vTemp= mods[incPaddle-1].rectHight;
  ...
  if(limiterR<moduleSize)   
 if(mods[limiterR-1].rectHight>mods[limiterR].rectHight)
  {
   vTemp= mods[limiterR-1].rectHight;
     mods[limiterR-1].rectHight=mods[limiterR].rectHight;
     mods[limiterR].rectHight=vTemp;
     }
 ...
 if(boolButton)
  {
   incPaddle++;
   limiterR++;    
  if(limiterR==moduleSize+1) {incPaddle=1;limiterR=moduleSize/2+1;}
