...
void draw() {
 ...
 // println(listOfCoordinatesY);  
  for (int i =0; i <listOfCoordinatesX.length-1;i++)
    { 
     if ( storX==listOfCoordinatesX[i] && storY==listOfCoordinatesY[i] )
      { overField=true; }
    }
 println(overField);  
}
...
class Module {
 ...
 int modColor=0;
  ...
  if(boolButton && mousePressed)
    { 
     if( x == listOfCoordinatesX[listOfCoordinatesX.length-1] &&
         y == listOfCoordinatesY[listOfCoordinatesY.length-1] )             
      {
      modColor=0; 
      }  
     } 
...
void mousePressed() {
 ...
 {
    storX=listOfCoordinatesX[listOfCoordinatesX.length-2];
    storY=listOfCoordinatesY[listOfCoordinatesY.length-2];
  }
}
void mouseDragged() {
  overField = false;
...
void mouseReleased() {
 ...
 if(boolButton)
 {
  knightX=storX;
  knightY=storY;  
  if(listOfCoordinatesX.length>1)
    {
